...
export type HyphenateTreeNode = { [l: string]: HyphenateTreeNode };
...
function interleave(word: string, points: Array<number>) {
  return _.map(
    _.zip(points, (word + " ").split("")),
    ([x, y]: [number, string]) => "" + x + y,
  ).join("");
}

export function hyphenate(
  tree: HyphenateTreeNode,
  word: string,
  listener: HyphenateListener = new NullListener(),
) {
